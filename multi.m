classdef multi % Of the form zn:[a1,a2,...,an]
%{
...
%}
     ...
     methods                                                  
         function self=multi(zn)         % Returns error if there are more than one input
             if nargin ~= 1
...
methods                                                      
    ...
    function mrep = matrep(self)                              % Matrix Representation 
       ...
       function out = plus(obj1,obj2)                           % Addition
           ...
           for i=1:siz(2)
               ...
               outfor(i)=multi(obj11.zn+obj21.zn);
           end
           out=outfor;
       end
       ...
       function out = times(self,other)                         % Multiplication
       ...
       function out = rdivide(self,other)                       % Division
       ...
       function out = power(input,p)                             % Power
   ...
   % use the standard matrix multiplication p times.
                ...
                out=multi(arrayM(matrep(input)^p));
   ...
   % built in atan function is employed. 
            ...
            elseif l==2
               ...
               half=l/2;
               L_1 = multi(input.zn(1:half));
               L_2 = multi(input.zn(half+1:end));
      ...
      function out = atan2(input2,input1)                           % Arctan2       
        ...
        l=length(input1.zn);
            ...
            if l == 1   % Again checking for a real number input.
                ...
                out=multi(atan2(input2.zn,input1.zn));
             ...
             % account the signs of the inputs.                      
                 ...
                 % small imginary components.
                     ...
                     if  abs(input.zn((l/2)+1)) < 10^-4
                         ...
                         if input.check_convergence == 'T' && modcheck(multi(input.zn(1:l/2)))
                        ...
                        outp1=atan2(multi(input.zn(1:half)),multi(1));
                        outp2=multi(input.zn(half+1:end))/(1+(multi(input.zn(1:half)))^2);
                                         
                        out=multi([outp1.zn(1:end),outp2.zn(1:end)]);
                        ...
                        outp=ar*0.5*log(multi(arrayM((matrep(ar+input))/(matrep(ar-input)))));
                        out=2*outp;
      ...
      function out = atan(input)                           % Arctan
        ...
        l=length(input.zn);
            ...
            if l == 1
                ...
                out=multi(atan(input.zn));
            
            elseif l==2

                
                outp=atan(input.zn(1)+(1i*input.zn(2)));             
                out=multi([real(outp),imag(outp)]);
               ...
               if  abs(input.zn((l/2)+1)) < 10^-7
                ...
                out=ar*0.5*log(multi(arrayM((matrep(ar+input))/(matrep(ar-input)))));
      ...
      function out = log(input)                               % Log
         ...
         % the roundoff error in the log(1+x) calculation.  
                ...
                if abs(input.zn(1)) > 0.999999999 && abs(input.zn(1)) < 1.0000000001 && abs(input.zn(2)) < 10^-10
                  if input.zn(2) > 0
                  out=multi([0.5*log1p(input.zn(2)^2),atan2(input.zn(2),input.zn(1))]);
                  else
                  out=multi([-0.5*log1p(input.zn(2)^2),atan2(input.zn(2),input.zn(1))]);
                  end
                else
                 out=multi([0.5*log(input.zn(1)^2+input.zn(2)^2),atan2(input.zn(2),input.zn(1))]);
                end
      ...
      function out = sin(input)                              % Sin
          ...
          l=length(input.zn);
            ...
            elseif l==2
                ...
                out=multi([(sin(input.zn(1))*cosh(input.zn(2))),(cos(input.zn(1))*sinh(input.zn(2)))]);
      ...
      function out = cos(input)                              % Cos
          ...
          l=length(input.zn);
             ...
             elseif l==2
                  ...
                  out=multi([(cos(input.zn(1))*cosh(input.zn(2))),-(sin(input.zn(1))*sinh(input.zn(2)))]);
      ...
      function out = sinh(input)                        % Sinh
          ...
          l=length(input.zn);
             ...
             elseif l==2
                  ...
                  out=multi([(sinh(input.zn(1))*cos(input.zn(2))),(cosh(input.zn(1))*sin(input.zn(2)))]);
      ...
      function out = cosh(input)                             % Cosh
           ...
           l=length(input.zn);
             ...
             elseif l==2
                  ...
                  out=multi([(cosh(input.zn(1))*cos(input.zn(2))),(sinh(input.zn(1))*sin(input.zn(2)))]);
          ...
          l=length(input.zn);
             ...
             elseif l==2
                 ...
                 if  abs(input.zn((l/2)+1)) < 10^-4
                        ...
                        outp1=asin(multi(input.zn(1:half)));
                        outp2=multi(input.zn(half+1:end))/((1-(multi(input.zn(1:half)))^2)^0.5);
                  ...
                  d1=multi(d.zn(1:half));
                  d2=multi(d.zn(half+1:end));
...
% multicomplex numbers into the same format. This function enables the
...
% numbers).
            ...
            if isa(self,'double')==1 && isa(other,'multi')==1
          
                 aa=size(other.zn)-1;
                 self = multi([self,zeros(1,aa(2))]);
        
            elseif isa(self,'multi')==1 && isa(other,'double')==1
                 
                 bb=size(self.zn)-1;
                 other = multi([other,zeros(1,bb(2))]);
        
            elseif isa(self,'multi')==1 && isa(other,'multi')==1
                 ...
                 bb=size(self.zn);
                      ...
                      if aa(2) > bb(2)
                          ...
                          self = multi([self.zn,zeros(1,(aa(2)-bb(2)))]);
                
                      elseif aa(2)<bb(2)   
                
                          other = multi([other.zn,zeros(1,(bb(2)-aa(2)))]);
                ...
                error('Inputs to function are not of double or multi type objects')
...
function out = modc(input)
    ...
    l=length(input1.zn);  
    input1=(((multi(input1.zn(1:l/2)))^(2))+(multi(input1.zn(l/2+1:end)))^(2))^(1/2);
...
function out = conj(input)
   % Complex conjugate 
    ...
    out = multi([input.zn(1:half),-input.zn(half+1:end)]);
